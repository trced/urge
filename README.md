...
urge. is not a wishlist. You write down what you *give up on*, at the moment you give it up. Thirty days later one question comes back — *do you still think about it?* — and you answer it in one word: forgotten, faintly, still.
...
There is one deliberate departure from the mock-up. It sketched a *reminder* — "mornings, 9 a.m.". An app with no server cannot wake a phone at nine, so that switch would have promised a notification the program can never send. It is replaced by a setting that is true: **ask the question** — when I open the app, or when I ask for it.
...
`verdict` is one of `forgotten`, `faint`, `still`, or `null` while the question is still owed. The words are stored neutral rather than translated: a file exported in French has to be readable in English, and "oublié" is a rendering, not a datum.
...
urge. adds **nothing** to the family palette. Its vocabulary is made of words — forgotten, faintly, still — and a verdict that needed a colour to be read would be a verdict that had not been written down properly.
...
- each answer is named in full too: "35 mm lens: still, I still want it" — three rows that read "forgotten / faintly / still" out of context say nothing
- colour is never the only carrier: there is no colour to carry anything
